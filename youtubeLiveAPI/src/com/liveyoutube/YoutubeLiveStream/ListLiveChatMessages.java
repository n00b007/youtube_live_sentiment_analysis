...
public class ListLiveChatMessages {
     ...
     *             from the chat associated with this video. If the videoId is not specified, the
     *             signed in
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err.println(
                    "GoogleJsonResponseException code: " + e.getDetails().getCode() + " : " + e
     ...
     */
    private static void listChatMessages(final String liveChatId, final String nextPageToken,
            long delayMs) {

        System.out.println(String
                .format("Getting chat messages in %1$.3f seconds...", delayMs * 0.001));
            ...
            public void run() {
                try {
                    ...
                    LiveChatMessageListResponse response =
                            youtube.liveChatMessages().list(liveChatId, "snippet, authorDetails")
                                    .setPageToken(nextPageToken).setFields(LIVE_CHAT_FIELDS)
                                    .execute();
                    ...
                    for (int i = 0; i < messages.size(); i++) {
                        ...
                        // Was using Java Redis before using kafka streams.
                        //JediTest.postJedis(buildOutput(snippet
                        // .getDisplayMessage(), message
                        // .getAuthorDetails(), snippet
                        // .getSuperChatDetails()));
                        // only sending the message to python
                        JediTest.postJedis(snippet.getDisplayMessage());
                        KafkaTest.sendMessage(snippet.getDisplayMessage());
                        System.out.println(
                                "The message sent to kafka is" + snippet.getDisplayMessage());
                        System.out.println(buildOutput(snippet.getDisplayMessage(), message
                                .getAuthorDetails(), snippet.getSuperChatDetails()));
                    }

                    // Request the next page of messages
                    listChatMessages(liveChatId, response.getNextPageToken(), response
    ...
    private static String getOnlyMessageWithPipeSeparator(String message,
            LiveChatMessageAuthorDetails author, LiveChatSuperChatDetails superChatDetails) {
